...
export const updateSearchCount = async (searchTerm, movie) => {
    if (!PROJECT_ID || !DATABASE_ID || !COLLECTION_ID) {
        console.warn("Appwrite credentials not configured. Search counts will not be tracked.");
        return;
    }
...
export const getTrendingMovies = async () => {
    if (!PROJECT_ID || !DATABASE_ID || !COLLECTION_ID) {
        console.warn("Appwrite credentials not configured. Skipping trending movies fetch.");
        return [];
    }
